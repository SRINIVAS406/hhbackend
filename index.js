...
mongoose.connect(
  "mongodb+srv://sri:sri@hhdev.amtukyd.mongodb.net/?retryWrites=true&w=majority"
);
  ...
  FormDataModel.findOne({ email: email }).then((user) => {
    if (user) {
      ...
      FormDataModel.create(req.body)
        .then((userobj) => {
          FormDataModel.updateOne(
            { _id: userobj.parentId },
            { $push: { child: userobj._id + "" } }
          )
            .then((result) => {
              console.log("-----", result);
  ...
  FormDataModel.deleteMany({ _id: { $in: userIds } })
    .then((result) => {
      ...
      if (result.deletedCount > 0) {
        ...
        FormDataModel.updateMany(
          { _id: { $in: userIds } },
          { $pull: { child: { $in: userIds } } }
        )
          .then(() => {
            console.log(
              `Users with IDs ${userIds.join(", ")} have been deleted.`
            );
...
app.put("/updateUser/:userId", (req, res) => {
  ...
  const { name, email, password, parentId, job, companyname, about, skill } =
    req.body;
